          ...
          <div>
            ...
            <div className="flex gap-2 mb-2.5">
              {['Hoy', 'Mañana', 'En 7 días'].map((label, i) => (
                <button
                  key={label}
                  onClick={() => quickSet(i === 0 ? 0 : i === 1 ? 1 : 7)}
            ...
            <div className="flex items-center gap-2">
              <input
                type="datetime-local"
                value={dueAt ? toLocalInput(dueAt) : ''}
                onChange={(e) => setDueAt(e.target.value ? fromLocalInput(e.target.value) : null)}
                className="flex-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 dark:focus:border-blue-500 transition-all"
              />
              {dueAt && (
                <button
