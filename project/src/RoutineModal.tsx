...
interface Props {
  ...
  onClose: () => void;
  onSave: (data: Omit<Routine, 'id' | 'streak' | 'lastDoneAt' | 'createdAt' | 'updatedAt'>) => void;
}

const FREQUENCIES: { key: Frequency; label: string; description: string }[] = [
  { key: 'daily',    label: 'Cada día',        description: 'Se repite todos los días' },
...
export function RoutineModal({ open, initial, onClose, onSave }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          ...
          <div>
            <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 block">Descripción (opcional)</label>
            <textarea
              value={description}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {FREQUENCIES.map((f) => (
                ...
                >
                  <span className={`text-sm font-medium ${frequency === f.key ? 'text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300'}`}>
                    {f.label}
                  </span>
                  <span className={`text-xs ${frequency === f.key ? 'text-blue-500 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'}`}>
                    {f.description}
                  </span>
                </button>
              ))}
          ...
          {(frequency === 'weekly' || frequency === 'custom') && (
            <div>
              <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 block">Días de la semana</label>
              ...
              {frequency === 'custom' && days.length < 2 && (
                <p className="mt-2 text-xs text-gray-400 dark:text-gray-500">Selecciona al menos 2 días para definir un rango personalizado.</p>
              )}
            </div>
          )}
        ...
        <div className="flex gap-2 px-5 pb-5 flex-shrink-0 border-t border-gray-100 dark:border-gray-800 pt-4">
          ...
          <button onClick={handleSave} disabled={!canSave} className="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 dark:disabled:bg-blue-900 text-sm font-medium text-white transition-colors">
