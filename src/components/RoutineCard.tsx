import {
  Check, Pencil, Trash2, Flame, RefreshCw, Briefcase, Sun, Calendar, CalendarDays, SlidersHorizontal,
} from 'lucide-react';
import type { Routine, Frequency } from '../types';
import { isDoneToday } from '../utils/routineUtils';
...
const FREQ_CONFIG: Record<Frequency, { label: string; icon: typeof RefreshCw; classes: string }> = {
  daily: {
    label: 'Cada día',
    icon: RefreshCw,
    classes: 'bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-900',
  },
  weekdays: {
    label: 'Entre semana',
    icon: Briefcase,
    classes: 'bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-900',
  },
  weekends: {
    label: 'Fines de semana',
    icon: Sun,
    classes: 'bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-900',
  },
  weekly: {
    label: 'Cada semana',
    icon: Calendar,
    classes: 'bg-violet-50 dark:bg-violet-950/40 text-violet-600 dark:text-violet-400 border border-violet-200 dark:border-violet-900',
  },
  monthly: {
    label: 'Cada mes',
    icon: CalendarDays,
    classes: 'bg-teal-50 dark:bg-teal-950/40 text-teal-600 dark:text-teal-400 border border-teal-200 dark:border-teal-900',
  },
  custom: {
    label: 'Personalizado',
    icon: SlidersHorizontal,
    classes: 'bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900',
  },
};
...
export function RoutineCard({ routine, onMarkDone, onEdit, onDelete }: Props) {
  ...
  const Icon = cfg.icon;
  const doneToday = isDoneToday(routine);
